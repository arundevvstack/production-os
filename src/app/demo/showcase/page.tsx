...
export default function ShowcasePage() {
  ...
  return (
    <div className="min-h-screen bg-muted dark:bg-primary selection:bg-emerald-500/30 font-sans">
      <CinematicHero onRunSimulation={runSimulation} />

      {/* Realtime Telemetry Wall */}
      <section className="py-20 border-y border-border dark:border-primary bg-white dark:bg-primary/50 backdrop-blur-xl relative overflow-hidden">
      ...
      <section className="py-24 bg-muted dark:bg-primary relative">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="relative rounded-2xl border border-border dark:border-primary bg-white dark:bg-primary/50 overflow-hidden shadow-2xl h-[600px] flex">
              ...
              <AnimatePresence>
                {simulationState === "spike" && (
                  <motion.div 
                    ...
                    className="absolute inset-0 z-10 bg-accent/10 pointer-events-none transition-colors duration-1000" 
                  />
                )}
                {simulationState === "degradation" && (
                  <motion.div 
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="absolute inset-0 z-10 bg-destructive/10 pointer-events-none transition-colors duration-1000" 
                  />
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="bg-muted dark:bg-primary border-border dark:border-primary">
              <CardContent className="p-6">
                ...
                <div className="text-xs text-muted-foreground mt-2 text-right">78% to Target</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-muted dark:bg-primary border-border dark:border-primary">
              <CardContent className="p-6">
                ...
                <div className="text-xs text-muted-foreground mt-2 text-right">-12% Optimization Mode Active</div>
              </CardContent>
            </Card>
  ...
  return (
    <Card className="bg-transparent border-none shadow-none">
      <CardContent className="p-0">
        <div className="flex items-center gap-2 mb-2 text-muted-foreground/80 dark:text-muted-foreground font-medium">
          {icon}
          {title}
        </div>
        <div className="text-4xl md:text-5xl font-bold text-primary dark:text-zinc-50 flex items-baseline tracking-tight">
          ...
          />
          <span className="text-2xl text-muted-foreground ml-1">{suffix}</span>
        </div>
      </CardContent>
    </Card>
  );
...
function IncidentLog({ active, time, title, desc, type }: { active: boolean, time: string, title: string, desc: string, type: "info" | "warning" | "critical" | "success" }) {
  ...
  return (
    <motion.div 
      ...
      className={`relative pl-4 border-l-2 ${active ? 'border-zinc-400 dark:border-zinc-600' : 'border-border dark:border-primary'} pb-2`}
    >
