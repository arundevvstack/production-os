...
import { CheckCircle2, Circle, MoreHorizontal, MapPin, Eye, Image as ImageIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Card } from '@/components/ui/card';
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@/components/ui/dropdown-menu';

export function LocationCard({ item, isSelected, onSelect, viewMode }: { item: any; isSelected: boolean; onSelect: (id: string) => void; viewMode: 'grid' | 'list' }) {
  const validation = item.metadata?.validation || {};
  const needsReview = validation.needs_review || validation.low_confidence || validation.spelling_changed || validation.scene_header;
  ...
  if (item.type) badges.push(<span key="type" className="px-2 py-0.5 bg-slate-100 text-slate-700 border border-slate-200 rounded-md text-[10px] font-bold uppercase">{item.type}</span>);
  if (item.time_of_day) badges.push(<span key="time" className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-md text-[10px] font-bold uppercase">{item.time_of_day}</span>);
  ...
  if (viewMode === 'list') {
    return (
      <div onClick={() => onSelect(item.id)} className={cn("flex items-center px-3 py-2 gap-4 cursor-pointer transition-all border-b group hover:bg-slate-50", isSelected ? "bg-red-50/40" : "")}>
        ...
        <div className="w-10 h-7 rounded bg-slate-100 shrink-0 overflow-hidden border flex items-center justify-center text-slate-400">
          {item.reference_images && item.reference_images.length > 0 ? <img src={item.reference_images[0]} alt={item.name} className="w-full h-full object-cover" /> : <MapPin className="w-3.5 h-3.5" />}
        </div>
        <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-3 flex flex-col min-w-0">
             <span className="text-[12px] text-slate-700 truncate">{item.type || 'Location'}</span>
          ...
          <div className="col-span-2 text-right text-[12px] font-semibold text-slate-500">
            {item.scene_count || item.scene_usage || 0} Scenes
          </div>
        </div>
  ...
  return (
    <Card className={cn("relative flex items-stretch cursor-pointer transition-all border group bg-white", isSelected ? "border-red-500 shadow-sm ring-1 ring-red-500" : "border-slate-200 hover:border-slate-300 hover:shadow-sm")}>
      ...
      {/* Left Thumbnail */}
      <div className="relative z-10 w-24 bg-slate-50 border-r border-slate-100 flex flex-col items-center justify-center p-1 shrink-0 pointer-events-none">
        <div className="w-full h-16 rounded overflow-hidden flex items-center justify-center text-slate-400 bg-slate-200 mb-1">
          {item.reference_images && item.reference_images.length > 0 ? <img src={item.reference_images[0]} alt={item.name} className="w-full h-full object-cover" /> : <MapPin className="w-6 h-6" />}
        </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-2.5 py-1.5 pointer-events-none min-w-0">
        ...
        <p className="text-[10px] text-slate-500 line-clamp-1 leading-tight mb-0.5">{item.description || "No description provided."}</p>
        
        <div className="flex items-center justify-between mt-auto pt-1.5 border-t border-slate-50">
          <div className="flex items-center gap-1 overflow-hidden">
            {badges}
          </div>
          <span className="text-[9px] font-bold text-slate-400 whitespace-nowrap shrink-0">{item.scene_count || item.scene_usage || 0} Sc</span>
        </div>
      </div>
