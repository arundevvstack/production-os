...
import { CheckCircle2, Circle, MoreHorizontal, Sparkles, Eye, Image as ImageIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Card } from '@/components/ui/card';
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@/components/ui/dropdown-menu';

export function VFXCard({ item, isSelected, onSelect, viewMode }: { item: any; isSelected: boolean; onSelect: (id: string) => void; viewMode: 'grid' | 'list' }) {
  ...
  if (item.cgi) badges.push(<span key="cgi" className="px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 rounded-md text-[10px] font-bold uppercase">CGI</span>);
  if (item.fire) badges.push(<span key="fire" className="px-2 py-0.5 bg-orange-50 text-orange-700 border border-orange-200 rounded-md text-[10px] font-bold uppercase">Fire</span>);
  if (item.explosions) badges.push(<span key="explosions" className="px-2 py-0.5 bg-red-50 text-red-700 border border-red-200 rounded-md text-[10px] font-bold uppercase">Explosions</span>);
      ...
      <Card onClick={() => onSelect(item.id)} className={cn("flex items-center p-3 gap-4 cursor-pointer transition-all border group", isSelected ? "border-red-500 bg-red-50/20 shadow-sm" : "border-slate-200 hover:border-slate-300 hover:shadow-sm")}>
        ...
        <div className="flex-1 min-w-0 grid grid-cols-5 gap-4 items-center">
          <div className="col-span-2">
            <h3 className="font-bold text-sm text-slate-800 truncate">{item.cgi || item.fire || item.explosions || item.name}</h3>
          </div>
      ...
      <div className="relative z-10 px-4 pt-2 flex flex-col gap-3 flex-1 pointer-events-none">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pt-1">
            <h3 className="font-bold text-lg text-slate-800 line-clamp-1">{item.cgi || item.fire || item.explosions || item.name}</h3>
            <p className="text-xs text-slate-500 truncate">VFX Element</p>
          </div>
        </div>
