...
import { CheckCircle2, Circle, MoreHorizontal, Lightbulb, Eye, Image as ImageIcon } from 'lucide-react';
import { cn } from '@/lib/utils';
import { Card } from '@/components/ui/card';
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@/components/ui/dropdown-menu';

export function LightingCard({ item, isSelected, onSelect, viewMode }: { item: any; isSelected: boolean; onSelect: (id: string) => void; viewMode: 'grid' | 'list' }) {
  ...
  if (item.mood) badges.push(<span key="mood" className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-md text-[10px] font-bold uppercase">{item.mood}</span>);
  if (item.time_of_day) badges.push(<span key="time" className="px-2 py-0.5 bg-slate-100 text-slate-700 border border-slate-200 rounded-md text-[10px] font-bold uppercase">{item.time_of_day}</span>);
      ...
      <Card onClick={() => onSelect(item.id)} className={cn("flex items-center p-3 gap-4 cursor-pointer transition-all border group", isSelected ? "border-red-500 bg-red-50/20 shadow-sm" : "border-slate-200 hover:border-slate-300 hover:shadow-sm")}>
        ...
        <div className="flex-1 min-w-0 grid grid-cols-5 gap-4 items-center">
          <div className="col-span-2">
            <h3 className="font-bold text-sm text-slate-800 truncate">{item.lighting_style || item.name}</h3>
          </div>
      ...
      <div className="relative z-10 px-4 pt-2 flex flex-col gap-3 flex-1 pointer-events-none">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pt-1">
            <h3 className="font-bold text-lg text-slate-800 line-clamp-1">{item.lighting_style || item.name}</h3>
            <p className="text-xs text-slate-500 truncate">Lighting Plan</p>
          </div>
        </div>
