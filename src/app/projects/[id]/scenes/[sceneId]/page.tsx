...
export default async function SceneDashboardPage({ params }: { params: Promise<{ id: string, sceneId: string }> }) {
  ...
  const statusInfo = workflowState.stages.find((s: any) => s.id === 'scenes');
  if (statusInfo?.locked) redirect(`/projects/${resolvedParams.id}`);

  const scene = await prisma.productionScene.findUnique({
    where: { id: resolvedParams.sceneId },
    include: {
      ProductionShot: { include: { ProductionPrompt: true, ProductionAsset: true }, orderBy: { shot_number: 'asc' } },
      ProductionReference: true,
      ProductionAsset: true
    }
  });

  if (!scene) redirect(`/projects/${resolvedParams.id}/scenes`);

  const totalShots = scene.ProductionShot.length;
  const completedShots = scene.ProductionShot.filter(s => true).length;
  const promptsDefined = scene.ProductionShot.filter((s: any) => s.ProductionPrompt.length > 0).length;
  ...
  const fakeChecklist = [
    ...
    { id: '4', content: 'References Added', is_completed: scene.ProductionReference.length > 0, is_required: true },
    ...
    <div className="w-full space-y-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="border rounded-xl bg-white p-4 shadow-sm">
          ...
          <div className="text-2xl font-bold">{scene.ProductionReference.length}</div>
        </div>
        <div className="border rounded-xl bg-white p-4 shadow-sm">
          <div className="text-slate-500 text-xs font-semibold uppercase mb-1">Scene Assets</div>
          <div className="text-2xl font-bold">{scene.ProductionAsset.length}</div>
        </div>
        <div className="border rounded-xl bg-white p-4 shadow-sm">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <section>
            ...
            <div className="space-y-4">
              {scene.ProductionShot.map((shot: any) => {
                const promptSet = shot.ProductionPrompt?.[0];
                return (
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex gap-4">
                          <div className="flex items-center gap-2">
                            ...
                            {promptSet ? (
                              <span className="text-green-600 bg-green-50 px-2 py-0.5 rounded text-xs font-bold uppercase">{"Draft"}</span>
                            ) : (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-slate-600 font-bold">{shot.ProductionAsset.length}</span>
                          </div>
                        </div>
                        {promptSet && (
                          <Link 
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {scene.ProductionReference.map(ref => (
          ...
          <section>
            ...
            <div className="space-y-3">
              {scene.ProductionAsset.map(asset => (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 text-[10px] uppercase font-bold rounded">v{"v1"}</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">{"No notes."}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <StatusBadge status={asset.type} />
                    {asset.id && (
                      <div className="h-6 w-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-[10px] shrink-0" title={asset.id}>
                        {asset.id.charAt(0)}
                      </div>
                    )}
                  </div>
              ...
              {scene.ProductionAsset.length === 0 && (
                <div className="p-8 text-center border-2 border-dashed rounded-xl text-slate-500 bg-white">
