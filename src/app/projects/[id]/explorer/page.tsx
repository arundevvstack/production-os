...
export default async function CreativeExplorerPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ q?: string }> }) {
  ...
  const searchResults = resolvedSearchParams.q ? await GraphEngine.universalSearch(project.id, resolvedSearchParams.q) : [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            ...
            <form className="relative" action={`/projects/${project.id}/explorer`}>
            ...
            {resolvedSearchParams.q && (
              <div className="mt-6 space-y-3">
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Graph Results for "{resolvedSearchParams.q}"</div>
              ...
              timeline.map(event => (
                <div key={event.id} className="relative pl-6">
                  ...
                  <div>
                    ...
                    {event.user && (
                      <div className="text-xs text-slate-500 mt-1">by {(event.user as any).fullName}</div>
                    )}
                  </div>
                </div>
              ))
