...
export default async function JobQueuePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jobs = await prisma.productionAIJob.findMany({
    where: { project_id: resolvedParams.id },
    include: {
            ...
            {jobs.map((job) => (
              <tr key={job.id} className="hover:bg-slate-50/50 transition cursor-pointer group">
                <td className="px-6 py-4">
                  <Link href={`/projects/${project.id}/jobs/${job.id}`} className="block">
                    <span className="font-mono font-medium text-blue-600 hover:underline">{job.id.substring(0, 8)}...</span>
                  </Link>
                </td>
                <td className="px-6 py-4">
                  <div className="font-medium text-slate-900">
                    {job.ProductionScene ? `S${job.ProductionScene.scene_number.toString().padStart(2, '0')}` : 'General'}
                    {job.ProductionShot ? ` - Shot ${job.ProductionShot.shot_number}` : ''}
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="font-semibold text-slate-900">{job.ProductionAIProvider.name}</div>
