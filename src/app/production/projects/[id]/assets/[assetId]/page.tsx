...
export default async function AssetDetailPage({ params }: { params: { id: string, assetId: string } }) {
  ...
  if (!asset || asset.versions.length === 0) redirect(`/production/projects/${params.id}/assets`);

  const currentVersion = asset.versions[0];
  const previousVersions = asset.versions.slice(1);
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between border-b pb-6">
        <div className="flex items-center gap-4">
          <Link href={`/production/projects/${params.id}/assets`} className="h-10 w-10 rounded-full border bg-white flex items-center justify-center hover:bg-slate-50 transition text-slate-500">
      ...
      <AssetReviewBar projectId={params.id} assetId={asset.id} currentStatus={asset.status} />
          ...
          <div className="border rounded-xl bg-white shadow-sm overflow-hidden">
            ...
            <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <div className="font-medium">{currentVersion.provider_id || currentVersion.job?.provider.name || 'Unknown'}</div>
              </div>
              <div>
              ...
              <div>
                ...
                {currentVersion.job_id ? (
                  <Link href={`/production/projects/${params.id}/jobs/${currentVersion.job_id}`} className="font-mono text-blue-600 hover:underline">
        ...
        <div className="space-y-6">
          <div className="border rounded-xl bg-white shadow-sm overflow-hidden">
            <div className="border-b p-4 bg-slate-50 flex items-center justify-between">
              ...
              <span className="text-xs font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{asset.versions.length}</span>
            </div>
